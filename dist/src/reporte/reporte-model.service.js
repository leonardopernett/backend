...
let ReportsModelService = class ReportsModelService {
    ...
    async getReportLecturaLimit(ini, fin, idpcrc, limite, pag) {
    ...
    async getReportComentarioLimit(ini, fin, idpcrc, limite, pag) {
        let result = await this.db.NIKSLAVE(`
        SELECT 
        articulo.id,
        articulo.title as titulo ,
    ...
    async getReportBaseDatosLimit(ini, fin, idpcrc, limite, pag) {
        let result = await this.db.NIKSLAVE(`
...
WHERE articulo.base_id=? LIMIT ? OFFSET ?`, [ini, fin, ini, fin, ini, fin,
    ...
    async getReportCambioLimit(ini, fin, idpcrc, limite, pag) {
        let result = await this.db.NIKSLAVE(`
    ...
    async getReportUsuarioLimit(idpcrc, limite, pag) {
        let result = await this.db.NIKSLAVE(`
    ...
    async getReportFinancieraLimit(ini, fin) {
        let result = await this.db.NIKSLAVE(`
        SELECT
        pcrc,
    ...
    async getReportObligatorioLimit(idpcrc, fechaini, fechafinal, usuarios, limite, pag) {
        let data = await this.db.NIKSLAVE(`SELECT a.user_name,a.documento,h.fecha_vista,f.titulo AS titulo_periodo,s.title AS titulo_articulo FROM articulos_obligatorio_usuario h
    ...
    async getReportLecturaCategoriaLimit(ini, fin, idcategoria, limite, pag) {
        let result = await this.db.NIKSLAVE(`
            SELECT articulo.id,
            articulo.title as titulo ,
            vista.usuario_id as cedula ,
    ...
    async getReportBaseDatosCategoriaLimit(ini, fin, idcategoria, limite, pag) {
        let result = await this.db.NIKSLAVE(`
            SELECT articulo.id,articulo.title as titulo,
    (SELECT count(vista.id) FROM vista WHERE vista.articulo_id=articulo.id AND DATE_FORMAT(vista.initial_date,'%Y/%m/%d') BETWEEN ? AND ?) as vista,
    (SELECT count(valoraciones.tipo_valoracion) FROM valoraciones WHERE valoraciones.articulo_id=articulo.id AND valoraciones.tipo_valoracion="like" AND ISNULL(valoraciones.final_date) AND DATE_FORMAT(valoraciones.creation_date,'%Y/%m/%d') BETWEEN ? AND ?) as util,
    ...
    async getReportComentariocategoriaLimit(ini, fin, idcategoria, limite, pag) {
        let result = await this.db.NIKSLAVE(`
            SELECT 
            articulo.id,
            articulo.title as titulo ,
    ...
    async getReportCambioCategoriaLimit(ini, fin, id, limite, pag) {
        let result = await this.db.NIKSLAVE(`
            SELECT articulo.id,articulo.title as titulo, tipoevento.evento, DATE_FORMAT(tipoevento.fecha_evento,'%d/%m/%Y')  as fecha ,categoria.name as categoria ,usuario.documento,usuario.user_name as usuarios
            FROM tipoevento JOIN articulo ON articulo.id=tipoevento.id_articulo
            INNER JOIN categoria ON categoria.id=articulo.categoria_id
    ...
    async getReportObligatorioCategoriaLimit(idpcrc, fechaini, fechafinal, usuarios, limite, pag, idcategoria) {
        let data = await this.db.NIKSLAVE(`SELECT a.user_name,a.documento,h.fecha_vista,f.titulo AS titulo_periodo,s.title AS titulo_articulo FROM articulos_obligatorio_usuario h
            JOIN articulos_obligatorios f ON f.id=h.id_periodo
            JOIN articulo s ON s.id=f.id_articulo
            JOIN usuario a ON a.id=h.id_usuario
    ...
    async getReportLecturaPcrc(ini, fin, idpcrc, limite, pag) {
        let result = await this.db.NIKSLAVE(`
        SELECT articulo.id,
        articulo.title as titulo ,
        vista.usuario_id as cedula ,
    ...
    async getLecturaTotal(ini, fin, idpcrc) {
        const limite = 5;
    ...
    async getLecturaCategoria(ini, fin, idcategoria) {
        const limite = 5;
        const limite2 = 100000;
        let result = await this.db.NIKSLAVE(`
    ...
    async getReportLecturaCategoria(ini, fin, idcategoria, limite, pag) {
        let result = await this.db.NIKSLAVE(`
        SELECT articulo.id,
        articulo.title as titulo ,
        vista.usuario_id as cedula ,
    ...
    async getReportComentarioPcrc(ini, fin, idpcrc, limite, pag) {
        let result = await this.db.NIKSLAVE(`
        SELECT 
articulo.id,
articulo.title as titulo ,
...
where articulo.base_id=? AND DATE_FORMAT(comentario.publication_date,'%Y-%m-%d') BETWEEN ? AND ? LIMIT ? OFFSET ?`, [idpcrc, ini, fin, limite, pag]);
    ...
    async getReportComentarioCategoria(ini, fin, idcategoria, limite, pag) {
        let result = await this.db.NIKSLAVE(`
        SELECT 
articulo.id,
articulo.title as titulo ,
...
where articulo.categoria_id=? AND DATE_FORMAT(comentario.publication_date,'%Y-%m-%d') BETWEEN ? AND ? LIMIT ? OFFSET ?`, [idcategoria, ini, fin, limite, pag]);
    ...
    async getComentarioTotal(ini, fin, idpcrc) {
        const limite = 5;
        const limite2 = 100000;
        let result = await this.db.NIKSLAVE(`
        SELECT
...
where articulo.base_id=? AND DATE_FORMAT(comentario.publication_date,'%Y-%m-%d') BETWEEN ? AND ?`, [idpcrc, ini, fin]);
    ...
    async getComentarioCategoriaTotal(ini, fin, idcategoria) {
        const limite = 5;
        const limite2 = 100000;
        let result = await this.db.NIKSLAVE(`
        SELECT
...
where articulo.categoria_id=? AND DATE_FORMAT(comentario.publication_date,'%Y-%m-%d') BETWEEN ? AND ?`, [idcategoria, ini, fin]);
    ...
    async getReportBaseDatosPcrc(ini, fin, idpcrc, limite, pag) {
        let result = await this.db.NIKSLAVE(`
        SELECT articulo.id,articulo.title as titulo,
(SELECT count(vista.id) FROM vista WHERE vista.articulo_id=articulo.id AND DATE_FORMAT(vista.initial_date,'%Y/%m/%d') BETWEEN ? AND ?) as vista,
(SELECT count(valoraciones.tipo_valoracion) FROM valoraciones WHERE valoraciones.articulo_id=articulo.id AND valoraciones.tipo_valoracion="like" AND ISNULL(valoraciones.final_date) AND DATE_FORMAT(valoraciones.creation_date,'%Y/%m/%d') BETWEEN ? AND ?) as util,
...
WHERE articulo.base_id=? LIMIT ? OFFSET ?`, [ini, fin, ini, fin, ini, fin,
    ...
    async getReportBaseDatosCategoria(ini, fin, categoria, limite, pag) {
        let result = await this.db.NIKSLAVE(`
        SELECT articulo.id,articulo.title as titulo,
(SELECT count(vista.id) FROM vista WHERE vista.articulo_id=articulo.id AND DATE_FORMAT(vista.initial_date,'%Y/%m/%d') BETWEEN ? AND ?) as vista,
(SELECT count(valoraciones.tipo_valoracion) FROM valoraciones WHERE valoraciones.articulo_id=articulo.id AND valoraciones.tipo_valoracion="like" AND ISNULL(valoraciones.final_date) AND DATE_FORMAT(valoraciones.creation_date,'%Y/%m/%d') BETWEEN ? AND ?) as util,
...
WHERE articulo.categoria_id=? LIMIT ? OFFSET ?`, [ini, fin, ini, fin, ini, fin,
    ...
    async getBaseDatosTotal(idpcrc) {
        const limite = 5;
        const limite2 = 500;
        let result = await this.db.NIKSLAVE(`
    ...
    async getBaseDatosCategoriaTotal(idcategoria) {
        const limite = 5;
        const limite2 = 500;
        let result = await this.db.NIKSLAVE(`
        SELECT COUNT(*) AS total_base
...
WHERE articulo.categoria_id=?`, [idcategoria]);
    ...
    async getReportCambioPcrc(ini, fin, idpcrc, limite, pag) {
        let result = await this.db.NIKSLAVE(`
        SELECT articulo.id,articulo.title as titulo, tipoevento.evento, DATE_FORMAT(tipoevento.fecha_evento,'%d/%m/%Y')  as fecha ,categoria.name as categoria ,usuario.documento,usuario.user_name as usuarios
FROM tipoevento JOIN articulo ON articulo.id=tipoevento.id_articulo
INNER JOIN categoria ON categoria.id=articulo.categoria_id
...
AND DATE_FORMAT(tipoevento.fecha_evento,'%Y-%m-%d') BETWEEN ? AND ? LIMIT ? OFFSET ?`, [idpcrc, ini, fin, limite, pag]);
    ...
    async getReportCambioCategoria(ini, fin, categoria, limite, pag) {
        let result = await this.db.NIKSLAVE(`
        SELECT articulo.id,articulo.title as titulo, tipoevento.evento, DATE_FORMAT(tipoevento.fecha_evento,'%d/%m/%Y')  as fecha ,categoria.name as categoria ,usuario.documento,usuario.user_name as usuarios
FROM tipoevento JOIN articulo ON articulo.id=tipoevento.id_articulo
INNER JOIN categoria ON categoria.id=articulo.categoria_id
...
AND DATE_FORMAT(tipoevento.fecha_evento,'%Y-%m-%d') BETWEEN ? AND ? LIMIT ? OFFSET ?`, [categoria, ini, fin, limite, pag]);
    ...
    async getCambioTotal(ini, fin, idpcrc) {
        const limite = 5;
        const limite2 = 100000;
        let result = await this.db.NIKSLAVE(`
    ...
    async getCambioCategoriaTotal(ini, fin, idcategoria) {
        const limite = 5;
        const limite2 = 100000;
        let result = await this.db.NIKSLAVE(`
        SELECT COUNT(*) AS total_cambio
    ...
    async getReportUsuarioPcrc(idpcrc, limite, pag) {
        let result = await this.db.NIKSLAVE(`
        SELECT usuario.documento,usuario.user_name as usuarios ,usuario.creation_date as fecha_creacion ,usuario.rol 
        FROM usuario
        JOIN usuario_base ON usuario_base.documento=usuario.documento
    ...
    async generaExcel(data, tipo, name) {
        const res = name;
        const url = `./public/uploads/${res}.xlsx`;
        const ws = XLSX.utils.json_to_sheet(data);
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, 'sheet1');
        XLSX.writeFile(wb, path_1.resolve(url));
    ...
    async uploadS3(url, name, type) {
        ...
        const { insertId } = await this.db.NIKSLAVE(`insert into exportables (name, tipo_id, url) values (?,?,?)`, [name, type, process.env.BUCKET_URL + '/' + parametro.Key]);
        const [resp] = await this.db.NIKSLAVE('select id, url from exportables where id=?', [insertId]);
        await fs_extra_1.unlink(path_1.resolve('./public/uploads/' + name + '.xlsx'));
        return resp;
    }
    async deleteReport(id) {
        const { name } = await this.db.NIKSLAVE('select name from exportables where id=?', [id]);
        const params = {
            Bucket: process.env.BUCKET_NAME,
            Key: 'folder/' + name + '.xlsx',
        };
    ...
    async depurarReporte() {
        const exportables = await this.db.NIKSLAVE('select id, name from exportables limit 100');
    ...
    async getObligatorioTotal(idpcrc, fechaini, fechafinal, usuarios) {
        const limite = 5;
        const limite2 = 500;
        let data = await this.db.NIKSLAVE(`SELECT COUNT(*) AS total_obligatorio FROM articulos_obligatorio_usuario h
    JOIN articulos_obligatorios f ON f.id=h.id_periodo
    JOIN articulo s ON s.id=f.id_articulo
    JOIN usuario a ON a.id=h.id_usuario
    JOIN articulo_obligatorio_pcrcs b ON b.id_periodo=f.id
    WHERE b.id_pcrc=? AND DATE_FORMAT(h.fecha_vista,'%Y/%m/%d') BETWEEN ? AND ?`, [idpcrc, fechaini, fechafinal]);
        if (usuarios == 1) {
            ...
            let datos = await this.db.NIKSLAVE(`SELECT COUNT(*) AS total_obligatorio FROM usuario a
          JOIN usuario_base b ON b.documento=a.documento
...
//# sourceMappingURL=reporte-model.service.js.map