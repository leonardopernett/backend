...
@Injectable()
export class PreturnoModelService {
    ...
    constructor(
        ...
        async insertarpreturno(data){
           ...
           /* let tokencosmos=await this.asignarTokenCosmos() */ 
        
          let [[dato]]:any=await this.db.nikcleanPoolConection.query('SELECT id FROM usuario WHERE documento=?',[data.cedula])
          
          let resp:any=await this.db.NIK(`insert into preturno 
               (titulo, descripcion, contenido ,fecha_inicial, fecha_final,validar_multiple,validar_unica,validar_concepto,tipo_preturno,creador) values (?,?,?,?,?,?,?,?,?,?) `,[
                ...
                dato.id
                    ...
                    await this.db.nikcleanPoolConection.query('INSERT INTO preturno_pcrcs (id_preturno,pcrc) VALUES ?',
                    [array]) 

                    /*  await this.notificarpreturno(tokencosmos,1,data.preturnotitulo,array) */ 
                  ...
                  if(data.tipopreturno==2){
                    ...
                    await this.db.nikcleanPoolConection.query('INSERT INTO acargo_usuario_preturno (id_preturno,cedula) VALUES ?',
                    [array]) 

                     /* await this.notificarpreturno(tokencosmos,2,data.preturnotitulo,array)  */
        ...
        async notificarpreturno(tokencosmo,tipo,titulo,array){
          ...
          if(tipo==1){
             ...
             let data:any=await this.db.nikcleanPoolConection.query(`SELECT a.documento FROM usuario_base a WHERE a.base_id IN (${nuevoarray.toString()})`)
             ...
             usuarios_red=usuario_red.map((item:any)=>item.usuario_red)
        ...
        async guardarPreguntas(idobligatorio,preguntas,respuestas){
          ...
          for (let index = 0; index < preguntasFinal.length; index++) {
               ...
               let data:any=await this.db.nikcleanPoolConection.query('INSERT INTO pregunta_preturno (pregunta,id_preturno) VALUES (?,?)',[preguntasFinal[index],idobligatorio])
       ...
       async guardarConcepto(idpreturno, preguntas, respuestas) {
        ...
        for (const [index, element1] of preguntas.entries()) {
            const data:any = await this.db.nikcleanPoolConection.query('INSERT INTO pregunta_concepto_preturno (pregunta,id_preturno) VALUES (?,?)', [element1, idpreturno]);
       ...
       async guardarPreguntasUnicas(idpreturno,preguntas,respuestas){
        ...
        for (let index = 0; index < preguntasFinal.length; index++) {
             ...
             let data:any=await this.db.nikcleanPoolConection.query('INSERT INTO pregunta_unica_preturno (pregunta,id_preturno) VALUES (?,?)',[preguntasFinal[index],idpreturno])
     ...
     async validarPreturnoTotal(pcrc, cedula) {
      ...
      let contar = [];
      let [data]: any = await this.db.nikcleanPoolConection.query(`SELECT 
          a.id_preturno
          FROM preturno a
          JOIN preturno_pcrcs b ON b.id_preturno=a.id_preturno
          WHERE 
      ...
      for (const element of contar) {
        let data: any = await this.db.NIK(`call preturno_validado(?,?)`, [idusuario.id, element]);
        if (data == undefined) {
          contador++;
        } else {
...
return data
    ...
    async guardarResultadoMultiple(id_preturno,cedula,porcentaje1,porcentaje2,porcentaje3){
      ...
      let [[dato]]:any=await this.db.nikcleanPoolConection.query('SELECT id FROM usuario WHERE documento=?',[cedula])
      let resultado:any=await this.db.nikcleanPoolConection.query(`SELECT a.intentos FROM respuesta_resultados_preturno a
      WHERE a.id_usuario=? AND a.id_preturno=?
      ORDER BY a.id_respuesta_resultado_preturno DESC LIMIT 1`,[dato.id,id_preturno])
      
      if(resultado[0].length==0){
        this.db.nikcleanPoolConection.query('INSERT INTO respuesta_resultados_preturno (id_usuario,porcentaje_multiple,porcentaje_unico,porcentaje_concepto,intentos,id_preturno) VALUES (?,?,?,?,?,?)',
        [dato.id, porcentaje1,porcentaje2,porcentaje3,1,id_preturno])
      }else{
        this.db.nikcleanPoolConection.query('INSERT INTO respuesta_resultados_preturno (id_usuario,porcentaje_multiple,porcentaje_unico,porcentaje_concepto,intentos,id_preturno) VALUES (?,?,?,?,?,?)',
        [dato.id, porcentaje1,porcentaje2,porcentaje3,resultado[0][0].intentos+1,id_preturno])
      } 

  }

  public async guardarCuestionarioMultiple(id_preturno,cedula){
...
public async guardarCuestionarioUnico(id_preturno,cedula){
...
public async guardarCuestionarioConcepto(id_preturno,cedula){
...
async validarRespuestasMultiple(respuestas){
  ...
  let data:any=await this.db.NIK('SELECT COUNT(*) as validar FROM respuestas_preturno a WHERE a.id_respuestas IN ('+respuestas.join()+') AND a.valida=1 ')
...
async vistoMultiple(id_preturno,cedula){
  ...
  let data:any=await this.db.NIK(`SELECT id FROM multiple_preturno a 
...
async vistoUnico(id_preturno,cedula){
  ...
  let data:any=await this.db.NIK(`SELECT id FROM unico_preturno a 
  WHERE a.id_usuario=? AND id_preturno=?`,[dato.id,id_preturno])
...
async vistoConcepto(id_preturno,cedula){
  ...
  let data:any=await this.db.NIK(`SELECT id FROM concepto_preturno a 
  WHERE a.id_usuario=? AND id_preturno=?`,[dato.id,id_preturno])
...
public async guardarVisto(id_preturno,cedula){
...
async visto(id_preturno,cedula){
  ...
  let data:any=await this.db.NIK(`SELECT id FROM validar_preturno a 
  WHERE a.id_usuario=? AND id_preturno=?`,[dato.id,id_preturno])
...
let [[dato]]:any=await this.db.nikcleanPoolConection.query('SELECT id FROM usuario WHERE documento=?',[cedula])
  ...
  let data:any=await this.db.NIK(`call preturno_validado(?,?)`,[dato.id,id_preturno])
...
async avance(cedula,pcrc){
  ...
  let [[dato]]:any=await this.db.nikcleanPoolConection.query('SELECT id FROM usuario WHERE documento=?',[cedula])
    ...
    let data:any=await this.db.NIK(`SELECT
    (
    (
   ...
   +(SELECT COUNT(*) FROM concepto_preturno e JOIN preturno_pcrcs n ON e.id_preturno=n.id_preturno JOIN preturno p ON p.id_preturno=n.id_preturno WHERE e.id_usuario=? AND p.borrado=0 AND p.tipo_preturno=1)
    ...
    let data2:any=await this.db.NIK(`
    SELECT
    (
    (
  ...
  async getReportPreturnoPcrc(pcrc,fechaini,fechafin,limite,page){
    ...
    let data:any=await this.db.NIK(`SELECT b.id_preturno,b.titulo,c.fecha_respuesta,d.documento,d.user_name,c.porcentaje_multiple,c.porcentaje_unico,c.porcentaje_concepto,c.intentos FROM preturno_pcrcs a
  ...
  public async getPreturnoTotal(ini,fin,idpcrc){
...
public async getReportPreturnoLimit(ini,fin,idpcrc,limite,pag){
...
public async Paginatotal(total_lecturas,limite){
  ...
  async personasAcargo(cedula){
    ...
    let data:any=await this.db.JARVIS(`SELECT a.documento,b.nombre_completo FROM dp_distribucion_personal a 
